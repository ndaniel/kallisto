...
class Multinomial {
    public:
        Multinomial(const std::vector<double>& counts, size_t seed = 42) :
            counts_(counts),
            gen_(seed),
         ...
         *
         */
        std::vector<double> sample(int nsamp) {
            if (nsamp < 1) {
                throw std::domain_error("nsamp must be -1 or >=1");
            }

            std::vector<double> samp(counts_.size(), 0.0);
        ...
        const std::vector<double>& counts() { return counts_; }
